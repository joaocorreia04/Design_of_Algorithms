...
public:
    Vertex(T in);
    Vertex(T in, double lon, double lat): info(in), latitude(lat), longitude(lon) {}
    ...
    double getDistTo(Vertex* dest);
    Edge<T> *findEdge(Vertex<T> *dest);

    double getLatitude() const { return latitude; }
    double getLongitude() const { return longitude; }

    std::vector<Vertex<T>*> sons;
protected:
    T info;                // info node
...
public:
    ...
    */
    ...
    bool addVertexCoords(const T &in, double lon, double lat);
    ...
    void addBidirectionalEdge(Vertex<T>* sourc, Vertex<T>* dest, double w);
    ...
    static Edge<T> *getEdge(Vertex<T> *v1, Vertex<T> *v2);
...
template <class T>
double Vertex<T>::getDistTo(Vertex *dest) {
...
template <class T>
bool Graph<T>::addVertexCoords(const T &in, double lon, double lat) {
...
bool Graph<T>::addEdge(const T &sourc, const T &dest, double w) {
    ...
    if (v1 == nullptr || v2 == nullptr){
        std::cout << "v1 or v2 is null" << std::endl;
        return false;
    }
    v1->addEdge(v2, w);
...
template <class T>
void Graph<T>::addBidirectionalEdge(Vertex<T>* sourc, Vertex<T>* dest, double w) {
    if (sourc == nullptr || dest == nullptr) {
        return;
    }
    sourc->addEdge(dest, w);
    dest->addEdge(sourc, w);
}
...
template <class T>
void Graph<T>::setFullyConnected(bool isFullyConnected) {
    this->fullyConnected = isFullyConnected;
}

template <class T>
double Graph<T>::getDist(unsigned int idOrig, unsigned int idDest) const {
    auto orig = findVertex(idOrig);
    auto dest = findVertex(idDest);
    if(orig == nullptr || dest == nullptr) return -1;

    for(const Edge<T>* e: (*orig).getAdj()){
        if(e->getDest()->getInfo() == idDest){
            return e->getWeight();
        }
    }
    if (this->hasCoords) {
        return haversine((*orig).getLatitude(), (*orig).getLongitude(), (*dest).getLatitude(), (*dest).getLongitude());
    }
    return -1;
}

template <class T>
double Graph<T>::haversine(double lat1, double long1, double lat2, double long2) {
...
template <class T>
Edge<T> *Graph<T>::getEdge(Vertex<T> *v1, Vertex<T> *v2) {
...
template <class T>
void Graph<T>::setHasCoords(bool hasCoords) {
    this->hasCoords = hasCoords;
}

template <class T>
bool Graph<T>::usesCoords() const {
    return hasCoords;
}

template <class T>
bool Graph<T>::isFullyConnected() const {
    return fullyConnected;
}

struct CompareEdges {
    bool operator()(Edge<int>* e1, Edge<int>* e2) const {
        if (e1->getWeight() == e2->getWeight()) return e1->getDest()->getInfo() > e2->getDest()->getInfo();
